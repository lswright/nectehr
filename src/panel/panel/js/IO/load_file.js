...
var olderActionSuiteResult = undefined;
var olderActionSuiteFile = undefined;
    ...
    reader.onload = function(event) {
        ...
        if (!checkIsVersion2(test_suite)) {
            if (test_suite.search("<table") > 0 && test_suite.search("<datalist>") < 0) {
                ...
                */
                // parse for testCase or actionSuite
                if (checkIsActionSuite(test_suite)) {
                    // alert("Sorry, we do not support Action Suite of the format of an early version of Selenium IDE now.");
                    olderActionSuiteResult = test_suite.substring(0, test_suite.indexOf("<table")) + test_suite.substring(test_suite.indexOf("</body>"));
                    olderActionSuiteFile = f;
        ...
        // append on test grid
        appendActionSuite(f, test_suite);
        return;
...
document.getElementById("load-actionSuite-hidden").addEventListener("change", function(event) {
...
document.getElementById("load-actionSuite-show").addEventListener("click", function(event) {
    event.stopPropagation();
    document.getElementById('load-actionSuite-hidden').click();
}, false);

document.getElementById("load-actionSuite-show-menu").addEventListener("click", function(event) {
    event.stopPropagation();
    document.getElementById('load-actionSuite-hidden').click();
}, false);
