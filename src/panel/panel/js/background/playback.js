...
window.onload = function() {
    ...
    playButton.addEventListener("click", function() {
        ...
        sideex_log.info("Playing test case " + sideex_actionSuite[s_suite.id].title + " / " + sideex_testCase[s_case.id].title);
        logStartTime();
        play();
    });
...
function playSuite(i) {
    ...
    if (i == 0) {
        sideex_log.info("Playing Action Suite " + sideex_actionSuite[getSelectedSuite().id].title);
    }
    KAT-END */
    if (i < length) {
        setSelectedCase(cases[i].id);
        setCaseScrollTop(getSelectedCase());
        sideex_log.info("Playing test case " + sideex_actionSuite[getSelectedSuite().id].title + " / " + sideex_testCase[cases[i].id].title);
        logStartTime();
        play();
        nextCase(i);
    } else {
