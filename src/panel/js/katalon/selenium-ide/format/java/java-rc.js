 ...
 *
 * @param actionSuite  the suite to format
 * @param filename   the file the formatted suite will be saved as
 */
function formatSuite(actionSuite, filename) {
    var suiteClass = /^(\w+)/.exec(filename)[1];
    suiteClass = suiteClass[0].toUpperCase() + suiteClass.substring(1);
    
    var formattedSuite = "import junit.framework.Test;\n"
        + "import junit.framework.ActionSuite;\n"
        + "\n"
        + "public class " + suiteClass + " {\n"
        + "\n"
        + indents(1) + "public static Action Suite() {\n"
        + indents(2) + "ActionSuite suite = new ActionSuite();\n";
        
    for (var i = 0; i < actionSuite.tests.length; ++i) {
        var testClass = actionSuite.tests[i].getTitle();
        formattedSuite += indents(2)
            + "suite.addActionSuite(" + testClass + ".class);\n";
    }
