...
function formatSuite(actionSuite, filename) {    
    formattedSuite = 'require "spec/ruby"\n' +
        ...
        for (var i = 0; i < actionSuite.tests.length; ++i) {
                    ...
                    if (typeof actionSuite.tests[i].filename != 'undefined') {
                        formattedSuite += 'require File.join(File.dirname(__FILE__),  "' + actionSuite.tests[i].filename.replace(/\.\w+$/,'') + '")\n';
                    } else {
                    // didn't load / save as a suite
                        var testFile = actionSuite.tests[i].getTitle();
