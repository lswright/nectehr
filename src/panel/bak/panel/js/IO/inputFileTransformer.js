...
function checkIsActionSuite(input) {
...
// for load in actionSuite
function transformActionSuiteVersion(str) {
...
function loadCaseIntoSuite(str) {
    let href = [];
    // find what testCase link is in the actionSuite
    ...
    openOldFileDialog("This is Action Suite of early version of Selenium IDE. Please load " + testCaseName)
        .then(function(answer) {
            if (answer == "true") {
                document.getElementById("load-older-actionSuite").click();
            } else {
                return ;
            }
    });
...
document.getElementById("load-older-actionSuite").addEventListener("change", afterLoadOlderTestCase, false);
...
function readOlderTestCase(file, index, filesLength) {
    ...
    reader.onload = function(event) {
        ...
        olderActionSuiteResult = appendOlderTestCase(event.target.result);
        if(index == filesLength-1) {
            appendActionSuite(olderActionSuiteFile, olderActionSuiteResult);
        } else {
            index += 1;
...
function appendOlderTestCase(str) {
    getSeleniumBase(str);
    let postindex = olderActionSuiteResult.indexOf("</body>");
    let fore = olderActionSuiteResult.substring(0, postindex);
    let back = olderActionSuiteResult.substring(postindex);
...
function appendActionSuite(suiteFile, suiteResult) {
    // append on test grid
    ...
    addActionSuite(suiteFileName, id);
